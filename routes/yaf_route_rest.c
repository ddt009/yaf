...
static zend_object_handlers yaf_route_rest_obj_handlers;
...
ZEND_BEGIN_ARG_INFO_EX(yaf_route_rest_match_arginfo, 0, 0, 1)
ZEND_ARG_INFO(0, uri)
ZEND_END_ARG_INFO()
/* }}} */

static HashTable *yaf_route_rest_get_properties(zval *object) /* {{{ */
{
	zval rv;
	HashTable *ht;
	yaf_route_rest_object *rewrite = Z_YAFROUTERESTOBJ_P(object);
...
static zend_object *yaf_route_rest_new(zend_class_entry *ce) /* {{{ */
{
	yaf_route_rest_object *rewrite = emalloc(sizeof(yaf_route_rest_object));

	zend_object_std_init(&rewrite->std, ce);
	rewrite->std.handlers = &yaf_route_rest_obj_handlers;
...
static void yaf_route_rest_object_free(zend_object *object) /* {{{ */
{
	yaf_route_rest_object *rewrite = (yaf_route_rest_object *)object;
...
static void yaf_route_rest_init(yaf_route_rest_object *rewrite, zend_string *match, zval *router, zval *verify) /* {{{ */
{
...
void yaf_route_rest_instance(yaf_route_t *route, zend_string *match, zval *router, zval *verify) /* {{{ */
{
	zend_object *rewrite = yaf_route_rest_new(yaf_route_rest_ce);

	yaf_route_rest_init((yaf_route_rest_object *)rewrite, match, router, verify);

	ZVAL_OBJ(route, rewrite);
}
/* }}} */

static int yaf_route_rest_match(yaf_route_rest_object *rewrite, const char *uri, size_t len, zval *ret) /* {{{ */
{
	char *pos, *m;
	uint32_t l;
...
int yaf_route_rest_route(yaf_route_t *route, yaf_request_t *req) /* {{{ */
{
	zval args;
	const char *req_uri;
	size_t req_uri_len;
	yaf_request_object *request = Z_YAFREQUESTOBJ_P(req);
	yaf_route_rest_object *rewrite = Z_YAFROUTERESTOBJ_P(route);
	...
	if (EXPECTED(yaf_route_rest_match(rewrite, req_uri, req_uri_len, &args)))
	{
		...
		//增加记录controller到zv_controller备用 20200918 by ddt009@fjddt.com
		zval zv_controller;
		ZVAL_STRINGL(&zv_controller, "", 0);
		...
		if ((controller = zend_hash_str_find(rewrite->router, ZEND_STRL("controller"))) && IS_STRING == Z_TYPE_P(controller))
		{
			if (Z_STRVAL_P(controller)[0] != ':')
			{
				...
				ZVAL_STRINGL(&zv_controller, Z_STRVAL_P(controller), Z_STRLEN_P(controller));
			}
			else
			{
				zval *c;
				if ((c = zend_hash_str_find(Z_ARRVAL(args), Z_STRVAL_P(controller) + 1, Z_STRLEN_P(controller) - 1)) && IS_STRING == Z_TYPE_P(c))
				{
					...
					ZVAL_STRINGL(&zv_controller, Z_STRVAL_P(c), Z_STRLEN_P(c));
				}
			}
		}
		...
		{
			//修改从rewrite复制的代码action为支持restful 20200918 by ddt009@fjddt.com
			...
			//取得method
			zend_string *zs_method_p = request->method;
			//zend_write(ZSTR_VAL(zs_method_p), ZSTR_LEN(zs_method_p));
			zval zv_method;
			ZVAL_STR(&zv_method, zs_method_p);

			zval t;
			ZVAL_STRING(&t, "_");
			concat_function(&zv_method, &zv_method, &t);
			zval_ptr_dtor(&t);

			zval zv_rest_action;
			ZVAL_STRINGL(&zv_rest_action, "", 0);

			if (ZSTR_VAL(zs_method_p)[0] == 'G')
			{
			...
			//action是数字，action_name为后一个参数或为空，把数字作为名为id的参数
			if (Z_STRVAL_P(action)[0] != ':')
			{
				//默认action
				//Method_action
				concat_function(&zv_rest_action, &zv_method, action);
			}
			else
			{
				//有action的情况会是:action传过来
				zval *a;
				if ((a = zend_hash_str_find(Z_ARRVAL(args), Z_STRVAL_P(action) + 1, Z_STRLEN_P(action) - 1)) != NULL && IS_STRING == Z_TYPE_P(a))
				{
					...
					if (is_numeric_string(Z_STRVAL_P(a), Z_STRLEN_P(a), &lval, &dval, 0) == IS_LONG)
					{
						...
						ZVAL_STRING(&url_delimieter, "/");

						//str_ci = /controller/action(id)/
						ZVAL_STRINGL(&str_ci, "", 0);
						concat_function(&str_ci, &str_ci, &url_delimieter);
						//concat_function(&str_ci, &str_ci, zend_read_property(yaf_request_ce, request, ZEND_STRL(request->controller), 1, NULL));
						concat_function(&str_ci, &str_ci, &zv_controller);
						concat_function(&str_ci, &str_ci, &url_delimieter);
						concat_function(&str_ci, &str_ci, a);
						concat_function(&str_ci, &str_ci, &url_delimieter);

						//uri = .../controller/action(id)/...
						ZVAL_STRINGL(&zval_request_uri, ZSTR_VAL(request->uri), ZSTR_LEN(request->uri));

						if (Z_STRVAL(zval_request_uri)[Z_STRLEN(zval_request_uri) - 1] != '/')
						{
							concat_function(&zval_request_uri, &zval_request_uri, &url_delimieter);
						}
						...
						//zv_rest_action = Method_(tempAction)
						concat_function(&zv_rest_action, &zv_method, &tempActionName);
					...
					{
						concat_function(&zv_rest_action, &zv_method, a);
					}
				}
			}

			//zend_update_property(yaf_request_ce, request, ZEND_STRL(request->action), &zv_rest_action);

			//php_printf("zv_rest_action=");
			//zend_print_zval(&zv_rest_action, 0);
			//php_printf("\n");

			yaf_request_set_action(request, Z_STR_P(&zv_rest_action));

			//zval_ptr_dtor(&zv_method);
			zend_string_release(zs_method_p);
			zval_ptr_dtor(&zv_controller);
			zval_ptr_dtor(&zv_rest_action);
		}
...
zend_string *yaf_route_rest_assemble(yaf_route_rest_object *rewrite, zval *info, zval *query) /* {{{ */
{
	zval pidents, *zv;
...
PHP_METHOD(yaf_route_rest, route)
{
	...
	RETURN_BOOL(yaf_route_rest_route(getThis(), request));
}
/** }}} */
...
PHP_METHOD(yaf_route_rest, match)
{
	...
	if (!yaf_route_rest_match(Z_YAFROUTERESTOBJ_P(getThis()), ZSTR_VAL(uri), ZSTR_LEN(uri), return_value))
	{
		RETURN_FALSE;
	}
}
...
PHP_METHOD(yaf_route_rest, __construct)
{
	...
	yaf_route_rest_init(Z_YAFROUTERESTOBJ_P(getThis()), match, route, verify);
}
/** }}} */
...
PHP_METHOD(yaf_route_rest, assemble)
{
	...
	if ((str = yaf_route_rest_assemble(Z_YAFROUTERESTOBJ_P(getThis()), info, query)))
	{
		RETURN_STR(str);
	}
}
...
zend_function_entry yaf_route_rest_methods[] = {
	PHP_ME(yaf_route_rest, __construct, yaf_route_rest_construct_arginfo, ZEND_ACC_PUBLIC | ZEND_ACC_CTOR)
		PHP_ME(yaf_route_rest, match, yaf_route_rest_match_arginfo, ZEND_ACC_PUBLIC)
			PHP_ME(yaf_route_rest, route, yaf_route_route_arginfo, ZEND_ACC_PUBLIC)
				PHP_ME(yaf_route_rest, assemble, yaf_route_assemble_arginfo, ZEND_ACC_PUBLIC){NULL, NULL, NULL}};
/* }}} */
...
{
	zend_class_entry ce;
	YAF_INIT_CLASS_ENTRY(ce, "Yaf_Route_Rest", "Yaf\\Route\\Rewrite", yaf_route_rest_methods);
	yaf_route_rest_ce = zend_register_internal_class_ex(&ce, NULL);
	yaf_route_rest_ce->ce_flags |= ZEND_ACC_FINAL;
	yaf_route_rest_ce->create_object = yaf_route_rest_new;
	yaf_route_rest_ce->serialize = zend_class_serialize_deny;
	yaf_route_rest_ce->unserialize = zend_class_unserialize_deny;

	zend_class_implements(yaf_route_rest_ce, 1, yaf_route_ce);

	memcpy(&yaf_route_rest_obj_handlers, zend_get_std_object_handlers(), sizeof(zend_object_handlers));
	yaf_route_rest_obj_handlers.free_obj = yaf_route_rest_object_free;
	yaf_route_rest_obj_handlers.clone_obj = NULL;
	yaf_route_rest_obj_handlers.get_gc = NULL;
	yaf_route_rest_obj_handlers.get_properties = yaf_route_rest_get_properties;
